...
public:
  ...
  typedef std::function<bool(double *, char **, int)> OnUpdate;
  Number(Property<Number> &main, OnUpdate on_update);
  single_property new_property(const std::string& name, const std::string& label, double min, double max, double step, double value, const std::string format = "%d");
  void fill_vector();
  void do_register() const;
  bool update(double *values, char *names[], int n);
