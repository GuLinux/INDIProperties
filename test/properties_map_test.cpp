...
TEST_F(Test_INDIPropertiesMap, Update) {
  char *names[] = {"prop name"};
  ISState state = ISS_OFF;
  string text = "hello";
  double num = 3;
  properties_map["hello"].add_number("number_name", nullptr, {"device name", "number name", "label", "group"}, [&](Number::vtype *v, char **, int){ num = v[0]; return true; } )
	    .add("prop name", "prop label 2", 1, 6, 2, num);
  properties_map["bla"].add_switch("switch_name", nullptr, {"device name", "switch name", "label", "group"}, ISR_1OFMANY, [&](Switch::vtype* v, char **, int){ state = v[0]; return true; } )
	    .add("prop name", "prop label", state);
  properties_map["foo"].add_text("text_name", nullptr, {"device name", "text name", "label", "group"}, [&](Text::vtype* v, char **, int){ text = v[0]; return true; } )
	    .add("prop name", "prop label 2", text.c_str());
  ASSERT_TRUE(properties_map.update("device name", "number name", new Number::vtype[1]{1}, names, 1));
  ASSERT_EQ(1, num);
  ASSERT_TRUE(properties_map.update("device name", "switch name", new Switch::vtype[1]{ISS_ON}, names, 1));
  ASSERT_EQ(ISS_ON, state);
  ASSERT_TRUE(properties_map.update("device name", "text name", new Text::vtype[1]{"world"}, names, 1));
